...
(defrecord UserRepositoryImpl [db-spec]
  ...
  (find-by-id [this id]
    (let [item (first (jdbc/query db-spec ["SELECT * FROM user WHERE u_id = ?" id]))]
      (model/->User (item :u_id) (item :u_login) (item :u_name) (item :u_password) (item :u_role))))

  (find-all [this]
    (let [items (jdbc/query db-spec ["SELECT * FROM user"])]
      (for [item items]
        (model/->User (item :u_id) (item :u_login) (item :u_name) (item :u_password) (item :u_role)))))

  (add [this model]
    (jdbc/insert! db-spec :user model))
  ...
  (find-by-login [this login]
    (let [item (first (jdbc/query db-spec ["SELECT * FROM user WHERE u_login = ?" login]))]
      (model/->User (item :u_id) (item :u_login) (item :u_name) (item :u_password) (item :u_role))))

  (find-by-login-and-hash [this login hash]
    (jdbc/query db-spec ["SELECT * FROM user WHERE u_login = ? AND u_password = ?" login hash])))
